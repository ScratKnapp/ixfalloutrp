	...
	hook.Add("CreateMenuButtons", "ixInventory", function(tabs) -- We use the same special ID because the idea is to completely override it.
		...
		tabs["inv"] = {
			...
			Create = function(info, container)
				...
				local maxWeight = character:GetMaxCarry()
				local overweight = character:Overweight()
					...
					local barO = weight:Add("DPanel")
						...
						barO.Paint = function(self)
							...
							if overweight == 1 then
								surface.SetDrawColor(Color(252, 186, 3)) -- Yellow for slightly over weight
							elseif overweight == 2 then
								surface.SetDrawColor(Color(255, 128, 0)) -- Orange for fairly over weight
							elseif overweight == 3 then
								surface.SetDrawColor(Color(205, 50, 50)) -- Red for max over weight
							end 
